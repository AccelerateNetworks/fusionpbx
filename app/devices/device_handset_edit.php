...
//action add or update
	if (isset($_REQUEST["id"])) {
		$action = "update";
		$device_handset_uuid = check_str($_REQUEST["id"]);
	}
	else {
		$action = "add";
	}
...
//get http post variables and set them to php variables
	if (count($_POST)>0) {
		...
		$device_handset_description = check_str($_POST["device_handset_description"]);
	}
...
if (count($_POST)>0 && strlen($_POST["persistformvar"]) == 0) {
	...
	if ($action == "update" && permission_exists('device_setting_edit')) {
		$device_handset_uuid = check_str($_POST["device_handset_uuid"]);
	}
		...
		if ($_POST["persistformvar"] != "true") {
			//add the device
				if ($action == "add" && permission_exists('device_setting_add')) {
					$sql = "insert into v_device_handsets ";
					$sql .= "(";
					$sql .= "device_uuid, ";
					...
					$sql .= "device_handset_description ";
					$sql .= ")";
					$sql .= "values ";
					$sql .= "(";
					$sql .= "'$device_uuid', ";
			...
			//update the device
				if ($action == "update" && permission_exists('device_setting_edit')) {
					$sql = "update v_device_handsets set ";
...
//pre-populate the form
	if (count($_GET)>0 && $_POST["persistformvar"] != "true") {
		...
		foreach ($result as &$row) {
			$device_handset = $row["device_handset"];
			$device_handset_slot = $row["device_handset_slot"];
			$device_sip_account = $row["device_sip_account"];
			$device_handset_description = $row["device_handset_description"];
...
//show the content
	...
	echo "</b></td>\n";
	echo "<td width='70%' align='right'><input type='button' class='btn' name='' alt='".$text['button-back']."' onclick=\"window.location='device_handsets.php?id=$device_uuid'\" value='".$text['button-back']."'></td>\n";
	echo "</tr>\n";
	echo "<tr>\n";
	...
	echo "	<input class='formfld' type='text' name='device_handset' maxlength='255' value=\"".escape($device_handset)."\">\n";
	echo "<br />\n";
	...
	echo "</td>\n";
	echo "<td class='vtable' align='left'>\n";
	echo "	<input class='formfld' type='text' name='device_handset_slot' maxlength='255' value=\"".escape($device_handset_slot)."\">\n";
	echo "<br />\n";
	...
	echo "</td>\n";
	echo "<td class='vtable' align='left'>\n";
	echo "	<input class='formfld' type='text' name='device_sip_account' maxlength='255' value=\"".escape($device_sip_account)."\">\n";
	echo "<br />\n";
	...
	echo "</td>\n";
	echo "<td class='vtable' align='left'>\n";
	echo "	<input class='formfld' type='text' name='device_handset_description' maxlength='255' value=\"".escape($device_handset_description)."\">\n";
	echo "<br />\n";
	...
	if ($action == "update") {
		echo "		<input type='hidden' name='device_handset_uuid' value='$device_handset_uuid'>\n";
	}
	echo "			<br>";
